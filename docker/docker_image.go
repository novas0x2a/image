...
// GetRepositoryTags list all tags available in the repository. Note that this
// has minimal connection with the tag(s) used for this specific
// ImageReference; the tag will be used to determine policy-based access (see
// PolicyConfigurationIdentity), but will otherwise be ignored. (This is a
// backward-compatible shim method which calls the module-level GetRepositoryTags)
...
// has minimal connection with the tag(s) used for this specific
// ImageReference; the tag will be used to determine policy-based access (see
// PolicyConfigurationIdentity), but will otherwise be ignored.
func GetRepositoryTags(ctx context.Context, sys *types.SystemContext, ref types.ImageReference) ([]string, error) {
	dr, ok := ref.(dockerReference)
	if !ok {
		return nil, errors.Errorf("ref must be an ImageReference")
	}
